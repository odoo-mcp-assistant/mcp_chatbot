...
- Per-tenant runtime values (LLM keys, prompt, MCP server URL, etc.) live in `ir.config_parameter` and are set via the Odoo Settings UI.
- Infrastructure-level secrets for the FastAPI sidecar — `API_BASE_URL`, `JWT_SECRET`, `JWT_ALGORITHM`, `JWT_AUDIENCE`, `JWT_TTL_SECONDS` — live in the addon's `.env` file and are read through `services/env_config.py`. They are **deliberately not exposed** in the Settings UI. OS environment variables override the `.env` file, so production deployments can inject them via systemd / container env. These values must match `mcp_chatbot_api/.env`.
